...
train_per = round(float(train_data.Aussentemperatur.count())/float(df_norm.Aussentemperatur.count())*100,2)


print("total data count: %d"%(df_norm.Aussentemperatur.count()))
...
test_start = df_norm.index.searchsorted(datetime(2014, 12, 1,0,1));
days_limit = 60
for step in range(1,days_limit+1):
	print("-------------------------");
	test_end = df_norm.index.searchsorted(datetime(2014, 12, 1,0,1)+timedelta(days=step));
	test_data = df_norm.ix[test_start:test_end];
	test_per = round(float(test_data.Aussentemperatur.count())/float(df_norm.Aussentemperatur.count())*100,2);
	...
	MSE = (mean_squared_error(test_actual, test_predictions)**0.5)*(df.Energie.max() - df.Energie.min()/df.Energie.mean());
...
pylab.title(regressor_name + "'s Error of predictions after time delta - Data Normalized")
...
plt.grid()
pylab.savefig('../img/'+regressor_name+'_day_error_scaled.png')
# pylab.show()
